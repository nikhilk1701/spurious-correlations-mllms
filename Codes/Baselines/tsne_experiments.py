...
def main():
    ...
    model = load_model(model, device, "/scratch/nk3853/pipeline_results0000/Run0023/Train/best.tar")
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=32, pin_memory=True, num_workers=1, shuffle=True)
    encodings = []
    label_to_num = {
        'waterbirdwater': 0,
        'waterbirdland': 1,
    ...
    num_to_label = { 0: 'waterbird', 1: 'landbird'}
    num_to_label_4c = {v: k for k, v in label_to_num.items()}  # Reverse mapping for legend
    ...
    # np.save(f'{run_dir}/image_encodings-small-finetuned23-all-test.npy', encodings)
    ...
    labels = [encoding['label'] for encoding in encodings]
    ...
    for label in np.unique(labels):
        ...
        scatter = plt.scatter(data_2d[indices, 0], data_2d[indices, 1], label=num_to_label_4c[label], alpha=0.7, s=5)
    ...
    plt.grid(True)
    plt.savefig("image_finetuned23-clip-small-all-test_4class.png")
