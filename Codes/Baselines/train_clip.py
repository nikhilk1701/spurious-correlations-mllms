# Code to: Train CLIP on waterbirds
...
class Train_CLIP(nn.Module):
    ...
    def __init__(self, model_type, dataset, text_classes, img_dir, config):
        ...
        if self.config.optimizer == 'adam':
            self.optimizer = AdamW(self.model.parameters(), weight_decay=self.config.weight_decay, lr=self.config.learning_rate)
    ...
    def init_dataloaders(self):
        ...
        self.train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.config.batch_size, pin_memory=True, num_workers=4, shuffle=False)

        val_dataset = CLIPDataloader(clip_transform= self.preprocess, learning_data= self.val_data)
        self.val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=self.config.batch_size, pin_memory=True, num_workers=4, shuffle=False)

        test_dataset = CLIPDataloader(clip_transform= self.preprocess, learning_data= self.test_data)
        self.test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=self.config.batch_size, pin_memory=True, num_workers=4, shuffle=False)
        return
    ...
    def train_model(self):
        ...
        for iteration in tqdm(range(start_iteration, total_iterations + 1)):
            ...
            images = images.unsqueeze(1).to(self.device)
            image_features = self.model.encode_image(images)
            image_features /= image_features.norm(dim=-1, keepdim=True)
            text_features = self.model.encode_text(self.tokenized_text_inputs).unsqueeze(0)
            text_features /= text_features.norm(dim=-1, keepdim=True)

            logits = (image_features @ text_features.T) * torch.exp(self.config.temperature)

            # Symmetric loss function
            labels = torch.arange(logits.shape[0]).to(self.device)
            loss_i = loss_func(logits, labels, axis=0)
            loss_t = loss_func(logits, labels, axis=1)
            ...
            del sampled_batch, images, image_features, logits
    ...
    def test_during_train(self):
        ...
        accuracy, class_accuracy = calculate_class_pred_accuracy_from_csv(self.text_classes, gt_labels, scores)

        self.test_dict['csv'][f'pred{self.current_iteration:04}'].append(accuracy, class_accuracy)
        self.test_dict['csv']['Image_Name'].append('Accuracy and classwise accuracy')
...
def main():
    ...
    model = Train_CLIP(model_type=config.model_type, dataset='waterbirds', text_classes=['landbird', 'waterbird'], img_dir='datasets', config=config)
    model.train_model()
