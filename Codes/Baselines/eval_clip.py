...
def save_to_csv(model_type, dataset, save_path, img_name, gt_labels, scores):
    df = pd.DataFrame({'img_name': img_name, 'gt_labels': gt_labels, 'scores': scores})
...
def calculate_class_pred_accuracy_from_csv(text_class_order, gt_labels, scores):
    ...
    for class_name in text_class_order:
        ...
        class_predictions = [class_predictions[i] for i in class_indices]
        correct_class_predictions = sum(pred == gt for pred, gt in zip(class_predictions, class_gt_labels))
...
def main():
    ...
    save_to_csv(model_type, dataset, save_path, img_name, gt_labels, scores)
    ...
    accuracy, class_accuracy = calculate_class_pred_accuracy_from_csv(text, gt_labels, scores)
